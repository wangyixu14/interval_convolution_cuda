...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	dim3 gridMul, blockMul;
    blockMul.x = blockMul.y = TILE_WIDTH; blockMul.z = 1;
    gridMul.x = ceil(P.width / (float)blockMul.x);
    gridMul.y = ceil(P.height / (float)blockMul.y);
    gridMul.z = 1;
	// dim3 gridMul(ceil(P.width / (float)TILE_WIDTH), ceil(P.height / (float)TILE_WIDTH), 1);
	// dim3 blockMul(TILE_WIDTH, TILE_WIDTH, 1);
	// Launch the device computation threads!
	MatrixMulKernel<<<gridMul, blockMul>>>(Md, Nd, Pd);
