...
int main(int argc, char* argv[])
{
    ...
    uint32_t *d_bins, *d_input;

    uint32_t *flatten_input = (uint32_t*)malloc(INPUT_HEIGHT*INPUT_WIDTH*sizeof(uint32_t));   
    for(int i=0; i<INPUT_HEIGHT; i++){
        for(int j=0; j<INPUT_WIDTH; j++){
            flatten_input[i*INPUT_WIDTH+j] = input[i][j]; 
        }
    }

    cudaMalloc((void **) &d_bins, HISTO_HEIGHT*HISTO_WIDTH*sizeof(uint32_t));
    cudaMalloc((void **) &d_input, INPUT_HEIGHT*INPUT_WIDTH*sizeof(uint32_t));
    cudaMemcpy(d_input, flatten_input, INPUT_HEIGHT*INPUT_WIDTH*sizeof(uint32_t), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(kernel_bins, d_bins, HISTO_HEIGHT*HISTO_WIDTH*sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaFree(d_input);
    cudaFree(d_bins);
    ...
    for (int i=0; i < HISTO_HEIGHT*HISTO_WIDTH; i++){
        if (gold_bins[i] != kernel_bins[i]){
            printf("Wrong Position at: %d  %d  %d \n", i, gold_bins[i], kernel_bins[i]);
            passed = 0;
            // break;
        }
    }
